...
const SignUp = () => {
    ...
    return (
        <div className="bg-bg font-body min-h-screen overflow-x-hidden">
            <div className="flex min-h-screen w-full relative">
                ...
                <section className="hidden lg:flex lg:w-1/2 relative overflow-hidden items-center justify-center p-20">
                    ...
                    <div className="relative z-10 max-w-lg text-center space-y-4">
                        <h1 className="font-display text-7xl italic text-text-muted tracking-tight leading-[1.1]">
                        ...
                        <div className="pt-6">
                            <div className="inline-block px-6 py-2 bg-primary text-on-primary hover:bg-primary-hover rounded-full border font-label text-sm tracking-[0.2em] uppercase glass-panel">
                                Moments Collective
                            </div>
                        </div>
                    </div>
                ...
                <section className="w-full lg:w-1/2 flex flex-col justify-center items-center p-6 md:p-12 lg:p-24 relative form">
                    <div className="w-full max-w-md space-y-10 relative z-10">
                        <header className="text-center ">
                            ...
                            <h2 className="font-display text-text text-3xl font-bold">Create your sanctuary</h2>
                            <p className="font-light">Begin your journey into the archives of memory.</p>
                        </header>
                        <div className="glass-panel p-4 md:p-4 shadow-2xl border border-border rounded-3xl">
                            <form className="form" onSubmit={signUpArea.handleSubmit}>
                                <div>
                                    ...
                                    <div className="space-y-1.5 mb-1.5">
                                        ...
                                        <input className="w-full border py-3 px-4 rounded-lg border-input-border outline-none text-text focus:ring-focus/20 bg-input-bg focus:ring-1 transition-all duration-500 placeholder:text-text-subtle" id="name" name="fullName" placeholder="Temiloluwa Ade" type="text" onChange={signUpArea.handleChange} onBlur={signUpArea.handleBlur} value={signUpArea.values.fullName} />
                                        {(signUpArea.touched.fullName && signUpArea.errors.fullName) && <small className='text-error'>{signUpArea.errors.fullName}</small>}

                                    </div>
                                    {/* <!-- Username Field --> */}
                                    <div className="space-y-1.5 mb-1.5">
                                        <label className="font-label text-xs uppercase tracking-widest ml-1" htmlFor="userName">Username</label>
                                        <input className="w-full border py-3 px-4 rounded-lg border-input-border outline-none text-text focus:ring-1 focus:ring-focus/20 bg-input-bg transition-all duration-500 placeholder:text-text-subtle" id="username" name="userName" placeholder="T3miii" type="text" onChange={signUpArea.handleChange} onBlur={signUpArea.handleBlur} value={signUpArea.values.userName} />
                                        {(signUpArea.touched.userName && signUpArea.errors.userName) && <small className='text-error'>{signUpArea.errors.userName}</small>}

                                    </div>
                                    {/* <!-- Gender --> */}
                                    <div className="space-y-1.5 mb-1.5">
                                        <label className="font-label text-xs uppercase tracking-widest ml-1" htmlFor="gender">Gender</label>
                                        <input className="w-full border py-3 px-4 rounded-lg border-input-border outline-none text-text focus:ring-focus/20 bg-input-bg focus:ring-1 transition-all duration-500 placeholder:text-text-subtle" id="gender" name="gender" placeholder="female" type="text" onChange={signUpArea.handleChange} onBlur={signUpArea.handleBlur} value={signUpArea.values.gender} />
                                    </div>
                                    {/* <!-- Email Field --> */}
                                    <div className="space-y-1.5 mb-1.5">
                                        <label className="font-label text-xs uppercase tracking-widest ml-1" htmlFor="email">Email Address</label>
                                        <input className="w-full border py-3 px-4 rounded-lg border-input-border outline-none text-text focus:ring-focus/20 bg-input-bg focus:ring-1 transition-all duration-500 placeholder:text-text-subtle" id="email" name="email" placeholder="temiade@gmail.com" type="email" onChange={signUpArea.handleChange} onBlur={signUpArea.handleBlur} value={signUpArea.values.email} />
                                        {(signUpArea.touched.email && signUpArea.errors.email) && <small className='text-error'>{signUpArea.errors.email}</small>}
                                    ...
                                    <div className="space-y-1.5 relative">
                                        ...
                                        {(signUpArea.touched.password && signUpArea.errors.password) && <small className='text-error'>{signUpArea.errors.password}</small>}<br />

                                    </div>
                                    <button className="w-full flex items-center justify-center bg-primary hover:bg-primary-hover gap-2 text-lg py-4 text-on-primary font-semibold rounded-full shadow-lg hover:scale-[1.02] disabled:opacity-70 disabled:hover:scale-100 disabled:cursor-not-allowed active:scale-95 transition-all duration-500 tracking-wide" type="submit" disabled={loader}>
                                        {loader ? (
                                            <>
                            ...
                            <div className="mt-8 grid grid-cols-2 gap-4 mb-6">
                                <button type="button" onClick={() => googleLogin()} disabled={loader} className="btn-glass flex items-center justify-center gap-3 py-3 px-4 rounded-full disabled:opacity-70 disabled:cursor-not-allowed">
                                ...
                                <button className="btn-glass flex items-center justify-center gap-3 py-3 px-4 rounded-full">
                                    <svg className="w-5 h-5" viewBox="0 0 24 24">
                        ...
                        <div className="text-center space-y-4">
                            <p className="font-light text-sm">
                                Already have an account? <Link to='/login'><span className='text-primary text-md font-medium'>Sign in</span></Link>
                            </p>
